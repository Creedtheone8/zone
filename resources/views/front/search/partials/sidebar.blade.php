		...
		<aside>
			...
			<div class="card" id="movableSidebarContent">
				<div class="card-body vstack gap-4 text-wrap">
					@include('front.search.partials.sidebar.post-type')
					...
					@endif
					@include('front.search.partials.sidebar.salary')
				</div>
			</div>
			...
			if (postTypeEls.length > 0) {
				postTypeEls.forEach((element) => {
					element.addEventListener('change', (e) => {
						...
						const queryStringEl = document.querySelector('input[type=hidden][name=postTypeQueryString]');
						if (!queryStringEl) {
							return false;
						}
