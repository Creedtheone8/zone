		...
		Route::prefix('auth')
			->group(function ($router) {
				...
				Route::controller(ForgotPasswordController::class)
					->group(function ($router) {
						Route::post('password/email', 'sendResetLink')->name('api.auth.password.email');
					});
				
				Route::controller(ResetPasswordController::class)
					->group(function ($router) {
						Route::post('password/token', 'sendResetToken')->name('api.auth.password.token');
				...
				Route::controller(SocialController::class)
					->group(function ($router) {
						...
						Route::get('{provider}', 'getProviderTargetUrl')->name('api.auth.social.connect');
						Route::get('{provider}/callback', 'handleProviderCallback')->name('api.auth.social.connect.callback');
					});
			});
...
Route::prefix('salaryTypes')
	...
	->group(function ($router) {
		$router->pattern('id', '[0-9]+');
		Route::get('/', 'index')->name('api.salaryTypes.index');
		Route::get('{id}', 'show')->name('api.salaryTypes.show');
	});
...
Route::prefix('users')
	...
	->group(function ($router) {
		...
		Route::middleware(['auth:sanctum'])->group(function ($router) {
			...
			Route::put('{id}/settings', 'updateSettings')->name('api.users.settings.update');
...
Route::prefix('companies')
	...
	->group(function ($router) {
		...
		Route::get('/', 'index')->name('api.companies.index');
		...
		Route::middleware(['auth:sanctum'])
			->group(function ($router) {
				...
				Route::post('/', 'store')->name('api.companies.store');
				Route::put('{id}', 'update')->name('api.companies.update');
				Route::delete('{ids}', 'destroy')->name('api.companies.destroy');
			});
	});
...
Route::prefix('resumes')
	...
	->group(function ($router) {
		...
		Route::get('/', 'index')->name('api.resumes.index');
		Route::get('{id}', 'show')->name('api.resumes.show');
		Route::middleware(['auth:sanctum'])
			->group(function ($router) {
				$router->pattern('id', '[0-9]+');
				$router->pattern('ids', '[0-9,]+');
				
				Route::post('/', 'store')->name('api.resumes.store');
				Route::put('{id}', 'update')->name('api.resumes.update');
				Route::delete('{ids}', 'destroy')->name('api.resumes.destroy');
			});
	});
	...
	->group(function ($router) {
		...
		Route::middleware(['auth:sanctum'])
			->group(function ($router) {
				...
				Route::put('{id}/company', 'updatePostCompany')->name('api.posts.update.company');
...
Route::prefix('paymentMethods')
	...
	->group(function ($router) {
		...
		Route::get('{id}', 'show')->name('api.paymentMethods.show');
	});
...
Route::prefix('posts')
	...
	->group(function ($router) {
		...
		Route::post('{id}/sendByEmail', 'sendPostByEmail')->name('api.posts.sendByEmail');
	});
