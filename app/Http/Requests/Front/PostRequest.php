...
class PostRequest extends Request
{
	...
	protected function prepareForValidation(): void
	{
		...
		if (isSingleStepFormEnabled()) {
			...
			if ($this->filled('company.name')) {
				...
				$input['company']['name'] = ensureContainsLetters($input['company']['name']);
			...
			if ($this->filled('company.description')) {
				...
				$input['company']['description'] = ensureContainsLetters($input['company']['description']);
	...
	public function attributes(): array
	{
		...
		if (isSingleStepFormEnabled()) {
			...
			$logoFile = $this->file('company.logo_path', $this->files->get('company.logo_path'));
			if (!empty($logoFile)) {
				$attributes['company.logo_path'] = trans('global.logo');
			}
		}
	...
	public function messages(): array
	{
		...
		if ($this->filled('parent_id') && !empty($this->input('parent_id'))) {
			$messages['category_id.required'] = trans('global.The field is required', ['field' => mb_strtolower(trans('global.Sub-Category'))]);
			$messages['category_id.not_in'] = trans('global.The field is required', ['field' => mb_strtolower(trans('global.Sub-Category'))]);
		}
