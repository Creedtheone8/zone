...
class CompanyController extends BaseController
{
	...
	public function index()
	{
		...
		// Meta Tags
		$title = trans('global.companies_list_title', ['appName' => config('settings.app.name')]);
		$description = trans('global.companies_list_description', ['appName' => config('settings.app.name')]);
		...
		$isFromSearchCompany = routeActionHas('Search\CompanyController');
		
		// SEO: noindex
		$noIndexCompaniesPages = (
			config('settings.seo.no_index_companies')
			&& routeActionHas('Search\CompanyController')
		);
	...
	public function profile($countryCode, $companyId = null)
	{
		...
		abort_if(empty($company), 404, $apiMessage ?? trans('global.company_not_found'));
