...
trait MetaTagTrait
{
	...
	public function getMetaTag(?array $preSearch = []): array
	{
		...
		// Init.
		$fallbackTitle = '';
		$fallbackTitle .= trans('global.jobs_offers');
	...
	private function applyLocationValue($location, &$title, &$description, &$keywords, &$fallbackTitle, &$fallbackDescription): void
	{
		...
		$fallbackDescription = trans('global.ads_in_location', ['location' => data_get($location, 'name')])
			. ', ' . config('country.name')
	...
	private function applyCompanyValue($company, &$title, &$description, &$keywords, &$fallbackTitle, &$fallbackDescription): void
	{
		...
		$fallbackTitle .= ' ' . trans('global.among') . ' ';
