...
class UserController extends PanelController
{
		...
		if ($this->onIndexPage) {
			...
			$this->xPanel->addFilter(
				...
				values: [
					...
					4 => trans('admin.has_recruiters_role'),
					5 => trans('admin.has_jobseekers_role'),
				],
				filterLogic: function ($value) {
					if ($value == 1) {
					...
					if ($value == 4) {
						$this->xPanel->addClause('role', Role::getRecruiterRole());
					}
					if ($value == 5) {
						$this->xPanel->addClause('role', Role::getJobseekerRole());
					}
				}
			);
		...
		*/
		if ($this->onCreatePage || $this->onEditPage) {
			...
			if (!$onAccountPage) {
				...
				// Link the 'user_type_id' to its corresponding role
				$recruiterRoleId = Role::getRecruiterRoleFromDb()->id ?? null;
				$jobseekerRoleId = Role::getJobseekerRoleFromDb()->id ?? null;
				$rolesByUserType = [];
				$userTypesByRole = [];
				
				if (!empty($recruiterRoleId) && !empty($jobseekerRoleId)) {
					$rolesByUserType = [
						UserType::RECRUITER->value => $recruiterRoleId,
						UserType::JOBSEEKER->value => $jobseekerRoleId,
					];
					
					$userTypesByRole = [
						$recruiterRoleId => UserType::RECRUITER->value,
						$jobseekerRoleId => UserType::JOBSEEKER->value,
					];
				}
				
				$data = [
					'rolesByUserType' => collect($rolesByUserType)->toJson(),
					'userTypesByRole' => collect($userTypesByRole)->toJson(),
				];
				$this->xPanel->addField([
					'name'  => 'javascript',
					'type'  => 'custom_html',
					'value' => view('admin.js.user', $data)->render(),
				]);
			}
		}
	}
	...
	private function handleInput(Request $request): Request
	{
		...
		// Handel 'roles' input
		$request = $this->handleRecruiterRolesFromUserTypeId($request);
		$request = $this->handleJobseekerRolesFromUserTypeId($request);
		
		// Handel 'user_type_id' input
		$request = $this->handleRecruiterUserTypeIdFromRoles($request);
		$request = $this->handleRecruiterUserTypeIdFromPermissions($request);
		$request = $this->handleJobseekerUserTypeIdFromRoles($request);
		$request = $this->handleJobseekerUserTypeIdFromPermissions($request);
	...
	private function userTypes(): array
	{
		...
		return collect($userTypes)->pluck('label', 'id')->toArray();
	}
