...
class PostController extends PanelController
{
		...
		*/
		if ($this->onIndexPage) {
			...
			$this->xPanel->addColumn([
				...
				'type'          => 'model_function',
				'function_name' => 'crudCompanyNameColumn',
			]);
		...
		*/
		if ($this->onCreatePage || $this->onEditPage) {
			...
			$this->xPanel->addField([
				...
				'hint'        => trans('global.notifications_channel_hint'),
				'wrapper'     => [
					'class' => 'col-md-6',
				],
			]);
	...
	private function uploadLogo($request)
	{
		...
		if (!empty($currentResourceId)) {
			$post = Post::with('company')->find($currentResourceId);
		...
		if (!empty($company)) {
			...
			GeneratePostLogoThumbJob::dispatchSync($company);
		}
		
		return $request;
	}
	
	public function salaryTypes(): array
	{
		$salaryTypes = SalaryType::query()->get();
		
		return $salaryTypes->pluck('name', 'id')->toArray();
	}
	...
	public function postTypes(): array
	{
		...
		return $postTypes->pluck('name', 'id')->toArray();
	}
}
