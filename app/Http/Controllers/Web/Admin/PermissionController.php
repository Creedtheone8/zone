...
class PermissionController extends PanelController
{
	...
	private function setPermissionDefaultRoles(Request $request): Request
	{
		...
		if (!empty($permission)) {
			...
			// recruiter
			if (in_array($permission->name, Permission::getRecruiterPermissions())) {
				$roles = Role::query()->where('name', '=', Role::getRecruiterRole());
				if ($roles->count() > 0) {
					$roleIdsFromDb = collect($roles->get())->keyBy('id')->keys()->toArray();
					$roleIds = array_merge($roleIds, $roleIdsFromDb);
				}
			...
			// jobseeker
			if (in_array($permission->name, Permission::getJobseekerPermissions())) {
				$roles = Role::query()->where('name', '=', Role::getJobseekerRole());
				if ($roles->count() > 0) {
					$roleIdsFromDb = collect($roles->get())->keyBy('id')->keys()->toArray();
					$roleIds = array_merge($roleIds, $roleIdsFromDb);
				}
