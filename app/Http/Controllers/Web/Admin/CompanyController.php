...
class CompanyController extends PanelController
{
		...
		*/
		if ($this->onIndexPage) {
			...
			$this->xPanel->addFilter(
				...
				values: getCountries(),
				filterLogic: fn ($value) => $this->xPanel->addClause('where', 'country_code', '=', $value)
			);
	...
	private function uploadLogo($request)
	{
		...
		// From edit page
		$currentResourceId = request()->route()->parameter('company');
		if (!empty($currentResourceId)) {
			$company = Company::find($currentResourceId);
		}
