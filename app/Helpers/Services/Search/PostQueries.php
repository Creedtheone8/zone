...
class PostQueries
{
	...
	public function fetch(): array
	{
		...
		GeneratePostCollectionLogoThumbsJob::dispatch($posts);
		...
		$companyId = $this->input['companyId'] ?? null;
		...
		if ($searchBasedOnCompany) {
			...
			if (empty($this->preSearch['company'])) {
				...
				// Get the Company
				$company = caching()->remember(Company::class, $cacheParams, function () use ($companyId) {
					return Company::find($companyId);
				});
				
				if (empty($company)) {
					$message = trans('global.company_not_found');
				}
