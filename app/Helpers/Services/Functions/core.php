...
{
	return (empty($user) || doesUserHaveRecruiterPermission($user));
}
...
function getCreateListingLinkInfo(): array
{
	...
	return [$linkUrl, $linkAttr, $userCanCreateListing];
}
...
function genEmailContactBtn($post = null, bool $btnBlock = false): string
{
	...
	$out .= '<a href="' . $btnLink . '" class="btn btn-secondary' . $btnClass . '" ' . $btnAttr . '>';
	$out .= '<i class="fa-regular fa-envelope"></i> ';
	$out .= trans('global.Apply Online');
	$out .= '</a>';
...
function getFilterClearBtn(?string $url): ?string
{
	$out = '';
	if (!empty($url)) {
		$linkClass = 'link-primary text-decoration-none link-opacity-75-hover';
