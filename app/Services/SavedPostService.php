...
class SavedPostService extends BaseService
{
	...
	public function store(Request $request): JsonResponse
	{
		...
		if ($savedPosts->count() > 0) {
			...
			$data['message'] = trans('global.Post deleted from favorites successfully');
		} else {
			// Store SavedPost
			$savedPostArray = [
			...
			$data['message'] = trans('global.Post saved in favorites successfully');
	...
	public function destroy(string $ids): JsonResponse
	{
		...
		if ($res) {
			...
			$count = count($ids);
			if ($count > 1) {
				$data['message'] = trans('global.x entities has been deleted successfully', ['entities' => trans('global.ads'), 'count' => $count]);
			} else {
				$data['message'] = trans('global.1 entity has been deleted successfully', ['entity' => trans('global.ad')]);
			}
		}
