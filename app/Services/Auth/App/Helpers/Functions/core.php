 ...
 * @return bool
 */
function doesUserHaveRecruiterPermission($authUser, bool $forceToFallbackOnErrorOccurred = false): bool
{
	$hasRecruiterPermissions = doesUserHavePermission($authUser, Permission::getRecruiterPermissions(), $forceToFallbackOnErrorOccurred);
	$userTypeId = $authUser->user_type_id ?? null;
	
	return ($userTypeId == UserType::RECRUITER->value || $hasRecruiterPermissions);
}
 ...
 * @return bool
 */
function doesUserHaveJobseekerPermission($authUser, bool $forceToFallbackOnErrorOccurred = false): bool
{
	$hasJobseekerPermissions = doesUserHavePermission($authUser, Permission::getJobseekerPermissions(), $forceToFallbackOnErrorOccurred);
	$userTypeId = $authUser->user_type_id ?? null;
	
	return ($userTypeId == UserType::JOBSEEKER->value || $hasJobseekerPermissions);
}
