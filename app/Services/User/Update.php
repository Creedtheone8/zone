...
trait Update
{
	...
	protected function updateDetails($id, UserRequest $request): JsonResponse
	{
		...
		$userTypeIdChanged = $request->filled('user_type_id') && $request->input('user_type_id') != $user->user_type_id;
		...
		// When the user' type is updated, assign the adequate role to the user
		if ($userTypeIdChanged) {
			// Get the user's type
			$isRecruiter = ($user->user_type_id == UserType::RECRUITER->value);
			$isJobseeker = ($user->user_type_id == UserType::JOBSEEKER->value);
			
			$recruiterRole = Role::getRecruiterRoleFromDb();
			$jobseekerRole = Role::getJobseekerRoleFromDb();
			
			// Assign the user's role related to the 'user_type_id' column
			if ($isRecruiter) {
				if (!empty($jobseekerRole) && !empty($jobseekerRole->name)) {
					if ($user->hasRole($jobseekerRole->name)) {
						$user->removeRole($jobseekerRole->name);
					}
				}
				
				if (!empty($recruiterRole) && !empty($recruiterRole->name)) {
					$user->removeRole($recruiterRole->name);
					$user->assignRole($recruiterRole->name);
				}
			}
			
			if ($isJobseeker) {
				if (!empty($recruiterRole) && !empty($recruiterRole->name)) {
					if ($user->hasRole($recruiterRole->name)) {
						$user->removeRole($recruiterRole->name);
					}
				}
				if (!empty($jobseekerRole) && !empty($jobseekerRole->name)) {
					$user->removeRole($jobseekerRole->name);
					$user->assignRole($jobseekerRole->name);
				}
			}
		}
