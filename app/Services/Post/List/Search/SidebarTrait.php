...
trait SidebarTrait
{
	...
	private function getPostTypes(array $params = []): array
	{
		...
		$cacheParams = [
			'action'  => 'get.postTypes',
			'locale'  => $locale,
			'orderBy' => 'lft',
		];
		
		$postTypes = caching()->remember(PostType::class, $cacheParams, function () {
	...
	private function orderByOptions(?array $city = [], array $params = []): array
	{
		...
		$orderByArray = [
			...
			[
				...
				'label'      => trans('global.salary_low_to_high'),
			],
			[
				'condition'  => true,
				'isSelected' => ($orderBy == 'salaryDesc'),
				'query'      => ['orderBy' => 'salaryDesc'],
				'label'      => trans('global.salary_high_to_low'),
			],
			[
