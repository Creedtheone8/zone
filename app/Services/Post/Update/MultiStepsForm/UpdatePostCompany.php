...
trait UpdatePostCompany
{
	...
	protected function changePostCompany($tokenOrId, CompanyRequest $request): JsonResponse
	{
		...
		if (empty($company)) {
			$message = trans('global.Please select a company or New Company to create one');
		...
		GeneratePostLogoThumbJob::dispatchSync($post);
		...
		$data = [
			'success' => true,
			'message' => trans('global.listing_company_updated'),
