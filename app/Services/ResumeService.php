...
class ResumeService extends BaseService
{
	...
	public function getEntries(array $params = []): JsonResponse
	{
		...
		$isForApplyingJob = castIntToBool($params['forApplyingJob'] ?? 0);
		...
		$resumes = PaginationHelper::adjustSides($resumes);
		...
		$message = ($resumes->count() <= 0) ? trans('global.no_resumes_found') : null;
	...
	public function getEntry($id, array $params = []): JsonResponse
	{
		...
		if (empty($resume)) {
			return apiResponse()->notFound(trans('global.resume_not_found'));
		}
	...
	public function store(ResumeRequest $request): JsonResponse
	{
		...
		$data = [
			'success' => true,
			'message' => trans('global.Your resume has created successfully'),
	...
	public function update($id, ResumeRequest $request): JsonResponse
	{
		...
		$data = [
			'success' => true,
			'message' => trans('global.Your resume has updated successfully'),
			'result'  => (new ResumeResource($resume))->toArray($request),
		];
	...
	public function destroy(string $ids): JsonResponse
	{
		...
		if ($res) {
			...
			$count = count($ids);
			if ($count > 1) {
				$data['message'] = trans('global.x entities has been deleted successfully', ['entities' => trans('global.resumes'), 'count' => $count]);
			} else {
				$data['message'] = trans('global.1 entity has been deleted successfully', ['entity' => trans('global.resume')]);
			}
		}
